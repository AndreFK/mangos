...
Arbol::Arbol(int x, int y) {
	...
	arbol.setPosition(x, 500);
	first = NULL;
	cant_M = 0;
	growth = 1;
}

void Arbol::addM(int peso, int x, int y) {
	if (growth == 2) {
		insert(peso, cant_M, x, y);
	}
}

void Arbol::addM(Mango*mango) {
	if (first == NULL) {
		first = mango;
	}
	else {
...
void Arbol::grow() {
	if (growth == 1) {
		...
		arbol.setPosition(arbol.getPosition().x, 500);
		arbol.setTextureRect(IntRect(0, 0, 102, 202));
		growth+=1;
	}
}
